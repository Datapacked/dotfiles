  ...
  in rec {
    ...
    modules = {
      ...
      mkPerUserModule = {
        ...
        mkAllUserConfig = mkPerUserConfig: (
          lib.mkIf cfg.enable (
            lib.mkMerge (lib.attrsets.mapAttrsToList (username: val: mkPerUserConfig username)
              cfg.users)
          )
        );
      in {
          ...
          generate_config = _cfg: mkAllUserConfig:
            ...
            // {
              home-manager.users = mkAllUserConfig (username: {
                "${username}" = {imports = home_manager_imports;};
              });
            };
        };
      ...
      mkSimpleDualModule = {
        ...
        modules.mkDualModule {
          inherit option_path description config home_manager_imports;
        };
    };
  };
}
